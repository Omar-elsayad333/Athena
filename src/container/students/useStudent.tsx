...
import { URL_TEACHERSTUDENTS_GROUP, URL_TEACHERSTUDENTS_INFO } from 'constant/url';
...
const useStudent = () => {
    ...
    const validation = () => {
        ...
        if(!selectedGroup.id || selectedGroup.id === studentData.groupId) {
            state = false
            setSelectedGroup({...selectedGroup, error: true, helperText: 'يجب اختيار مجموعه جديده'})
        }
        return state
    }
